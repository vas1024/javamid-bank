...
для алерта с курсами валют таймер не подошел, так как он обновляется только когда после долгого перерыва приходит обновление,
...
курсы обновляются раз в 10 секунд, прометеус скрейпит данные раз в 15 секунд.
таким образом, при нормальной работе гаудж показывает случайное число 0..10 ( на самом деле периодически 2 числа )
...
но получилось это только с логстешем. это красиво как в курсе, с вольюс файлом, но неудобно, неидемпотентно, 
скрипт падает, если гит уже скачан и если helm install уже есть. без хелма удобнее.. 
